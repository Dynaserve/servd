// Command platform is the Servd control-plane API. This build is dependency-free
// and file-backed (no MongoDB or Docker required): it persists services to a
// JSON file and serves the REST API the frontend consumes.
//
...
func main() {
	...
	proxy := NewProxyManager(publicHost, 9000, 9100)
	restoreProxies(store, proxy)
	...
	docker := newDockerctl(env("APPS_NETWORK", "servd-apps"))
	dctx, dcancel := context.WithTimeout(context.Background(), 5*time.Second)
	if docker.available(dctx) {
		if err := docker.ensureNetwork(dctx); err != nil {
			log.Printf("docker network setup failed: %v", err)
		} else {
			deployer = NewDeployer(store, docker, proxy, encKey, githubApp)
			deployer.Prewarm() // pre-pull base images so first builds start warm
			builder := "Dockerfile buildpack"
			if deployer.railpack != nil {
...
func restoreProxies(store Storer, proxy *ProxyManager) {
	all, err := store.AllServices()
	if err != nil {
	...
	for _, svc := range all {
		...
		if exposed && target != "" {
			if _, err := proxy.Expose(idOf(svc), target); err != nil {
				log.Printf("restore proxy for %s failed: %v", idOf(svc), err)
			}
		}
	}
}
